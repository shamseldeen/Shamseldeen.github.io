...
def build(lang):
    ...
    title = t("Food analysis, results and notebook | Shams Insights", "تحليل الأغذية الكامل والكود | شمس إنسايتس")
    ...
    parts.append(f'''<section class="learning-hero">
      ...
      <h1>{t('The Food study.', 'تحليل الأغذية.')}<br><em>{t('Every step, in one place.', 'كل خطوة، في مكان واحد.')}</em></h1>
      <p class="lead">{t('Explore the data, inspect the results and open the Python code as you read. This page brings the reproducible 2026 review and the archived 2023 notebook into the portfolio.', 'استكشف البيانات والنتائج، وافتح كود Python أثناء القراءة. تجمع هذه الصفحة مراجعة 2026 القابلة لإعادة التشغيل ودفتر 2023 المؤرشف داخل البورتفوليو.')}</p>
